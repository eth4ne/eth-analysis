  ...
  try {
    ...
    for (let i = 0; i < accesslist.length; i++) {
      if (accesslist[i].storageKeys.length === 0) {
        await conn.query("INSERT INTO `transactions_accesslist` (`blocknumber`, `transactionindex`, `accesslistindex`, `address`) VALUES (?, ?, ?, UNHEX(SUBSTRING(?, 3)));", [tx.blockNumber, tx.transactionIndex, i, accesslist[i].address]);
      } else {
        for (let j in accesslist[i].storageKeys) {
          await conn.query("INSERT INTO `transactions_accesslist` (`blocknumber`, `transactionindex`, `accesslistindex`, `address`, `storagekeys`) VALUES (?, ?, ?, UNHEX(SUBSTRING(?, 3)), UNHEX(SUBSTRING(?, 3)));", [tx.blockNumber, tx.transactionIndex, i, accesslist[i].address, accesslist[i].storageKeys[j]]);
        }
      }
    }
  } catch (err) {
